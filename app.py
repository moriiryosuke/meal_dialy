...
meal_df = pd.read_csv("static/csv/meal.csv")

nut_gol_df = pd.read_csv("static/csv/nut_gol.csv",index_col=0)
...
model = load_model("75_num013_model.h5")
...
nutl_ratio = {"kcal":0,"protein":0,"fat":0,"carbs":0,"vege":0}
...
def index():
    ...
    today_nutl["kcal"] = round(today_nutl["kcal"],2)
    today_nutl["protein"] = round(today_nutl["protein"],2)
    today_nutl["fat"] = round(today_nutl["fat"],2)
    today_nutl["carbs"] = round(today_nutl["carbs"],2)
    today_nutl["vege"] = round(today_nutl["vege"],2)

    for i in users:
        nutl_ratio["kcal"] = round(today_nutl["kcal"]/i.kcal*100,1)
        nutl_ratio["protein"] = round(today_nutl["protein"]/i.protein*100,1)
        nutl_ratio["fat"] = round(today_nutl["fat"]/i.fat*100,1)
        nutl_ratio["carbs"] = round(today_nutl["carbs"]/i.carbs*100,1)
        nutl_ratio["vege"] = round(today_nutl["vege"]/i.vege*100,1)

    return render_template('index.html',posts = posts,users = users,nutl=today_nutl,ratio=nutl_ratio)
...
def update(id):
    ...
    if request.method =="GET":
        ...
        post.meal_time = request.form.get("meal_time")
        db.session.commit()
        return redirect("/")
