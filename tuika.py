...
for meal in classes:
    ...
    gram = nutl_df.loc[meal,nutl_colum[6]]
    print(nutl_df.loc[meal,nutl_colum[5]])
    with app.app_context():
        post = Nutrients(meal=meal,kcal=kcal,protein=protein,fat=fat,carbs=carbs,salt=salt,vege=vege,gram=gram)
        db.session.add(post)
        db.session.commit()
